...
REGLA CRÍTICA: SIEMPRE inicia pidiendo consentimiento, sin saludos ni texto extra.
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if current_state["qa"].get("done"):
            ...
            # Mostrar estructuras finales bonitas
            print("📦 Estructura clínica (qa['structured']):")
            print(json.dumps(current_state["qa"].get("structured", {}), indent=2, ensure_ascii=False))
            print("\n🩺 Síntomas canónicos (qa['symptoms']):")
            print(current_state["qa"].get("symptoms", []))
            break
